...
static void	fill_philo_i(t_philo **p_arr_philo, t_philo_stat philo_stat,
				t_forks forks, int i);
			...
			t_forks forks, t_philo **p_arr_philo)
{
	...
	*p_arr_philo = malloc(philo_stat.nb_philo * sizeof(t_philo));
	...
	{
		fill_philo_i(p_arr_philo, philo_stat, forks, i);
		i++;
	}
	return (0);
}
				...
				t_forks forks, int i)
{
	...
	(*p_arr_philo)[i].p_start_mutex = &forks.start_mutex;
}
